...
namespace BoltSystem\Yii2Logs\log\error\migrations;
...
class m250226_102344_create_error_log_table extends Migration
{
    ...
    public function up()
    {
        ...
        $this->createTable('{{%logs_error}}', [
            ...
            'level'       => $this->string(250)->notNull()->default('')->comment('Уровень'),
            'code'        => $this->string(250)->notNull()->default('')->comment('Код'),
            'description' => $this->text()->null()->comment('Описание'),
            'meta'        => $this->json()->null()->comment('META'),
            'user_id'     => $this->integer()->notNull()->default(0)->comment('Пользователь'),
